...
#pragma omp target data map(to:mat)
	...
	while (!done && (cnt_iter < MAX_ITER)) {
		...
		#pragma omp metadirective when(device={arch("nvptx64")}: target teams distribute parallel for collapse(2) reduction(+:diff) private(temp, x)) default(parallel for collapse(2) reduction(+:diff) private(temp, x))
		for (int i = 1; i < n-1; i++) {
			for (int j = 1; j < n-1; j++) {
				temp = mat[i][j];
