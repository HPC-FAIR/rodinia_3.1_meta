...
{
	return sqroot(float(GAMMA)*pressure/density);
}
...
#pragma omp simd
	for(int i = b_start; i < b_end; i++)
	{
		...
		step_factors[i] = float(0.5f) / (sqroot(areas[i]) * (sqroot(speed_sqd) + speed_of_sound));
	}
        }
}
...
#pragma omp simd
	for(int i = b_start; i < b_end; ++i)
	{
		...
		float speed_i                              = sqroot(speed_sqd_i);
...
#pragma unroll
		for(int j = 0; j < NNB; j++)
		{
			...
			normal_len = sqroot(normal.x*normal.x + normal.y*normal.y + normal.z*normal.z);
			...
			if(nb >= 0) 	// a legitimate neighbor
			{
				...
				factor = -normal_len*smoothing_coefficient*float(0.5f)*(speed_i + sqroot(speed_sqd_nb) + speed_of_sound_i + speed_of_sound_nb);
