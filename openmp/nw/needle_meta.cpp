...
#endif
//#pragma omp metadirective when(device={arch("nvptx64")}: declare target)
int maximum( int a,
		 int b,
		 int c){
...
//#pragma omp metadirective when(device={arch("nvptx64")}: end declare target)
#ifdef OMP_OFFLOAD
...
#pragma omp metadirective when(device={arch("nvptx64")}: target data map(to: max_cols, penalty, referrence[0:transfer_size]) map(input_itemsets[0:transfer_size])) default(parallel num_threads(1))
    //#pragma omp target data map(to: max_cols, penalty, referrence[0:transfer_size]) map(input_itemsets[0:transfer_size])
    {

#pragma omp metadirective when(device={arch("nvptx64")}: target private(blk)) default(parallel num_threads(1))
    //#pragma omp target 
//#endif
    for( blk = 1; blk <= (max_cols-1)/BLOCK_SIZE; blk++ )
    {
      int blk1 = 0;
#ifdef OPENMP
#pragma omp parallel for schedule(static) shared(input_itemsets, referrence) firstprivate(blk, max_rows, max_cols, penalty) private(b_index_y, input_itemsets_l, reference_l, i, j)
#endif
        for( int b_index_x = 0; b_index_x < blk; ++b_index_x)
        {
            int b_index_y1;
            int input_itemsets_l1[(BLOCK_SIZE + 1) *(BLOCK_SIZE+1)] __attribute__ ((aligned (64)));
            int reference_l1[BLOCK_SIZE * BLOCK_SIZE] __attribute__ ((aligned (64)));        
            b_index_y = blk - 1 - b_index_x;
...
#ifdef OMP_OFFLOAD
    #pragma omp target private(blk)
#endif
    for ( blk = 2; blk <= (max_cols-1)/BLOCK_SIZE; blk++ )
    {
#ifdef OPENMP
#pragma omp parallel for schedule(static) shared(input_itemsets, referrence) firstprivate(blk, max_rows, max_cols, penalty) private(i,j, b_index_y, input_itemsets_l, reference_l)
#endif
        for( int b_index_x = blk - 1; b_index_x < (max_cols-1)/BLOCK_SIZE; ++b_index_x)
        {
            ...
            b_index_y = (max_cols-1)/BLOCK_SIZE + blk - 2 - b_index_x;
