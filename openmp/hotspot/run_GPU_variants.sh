...
do
  ...
  echo "Test command: $GPUPERFTOOL  $GPUPERFFLAG -u ms --log-file $LOGDIR/${t}_${inputdata}.log $app $inputdata $inputdata 2 $t $tempinut $powerinput output.out"
  $GPUPERFTOOL  $GPUPERFFLAG -u ms --log-file "$LOGDIR/${t}_${inputdata}.log" $app $inputdata $inputdata 2 $t $tempinut $powerinput output.out &> tmp.log
