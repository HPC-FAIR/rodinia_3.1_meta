...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(weights, Nparticles) private(x))
	...
	#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(arrayX, arrayY, xe, ye) private(x))
	for(x = 0; x < Nparticles; x++){
		arrayX[x] = xe;
		arrayY[x] = ye;
	}
	...
	for(k = 1; k < Nfr; k++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(arrayX, arrayY, Nparticles, seed) private(x))
		for(x = 0; x < Nparticles; x++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(likelihood, I, arrayX, arrayY, objxy, ind) private(x, y, indX, indY))
		for(x = 0; x < Nparticles; x++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(Nparticles, weights, likelihood) private(x))
		for(x = 0; x < Nparticles; x++){
		...
		double sumWeights = 0;
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for private(x) reduction(+:sumWeights))
		for(x = 0; x < Nparticles; x++){
			sumWeights += weights[x];
		}
		long long sum_time = get_time();
		printf("TIME TO SUM WEIGHTS TOOK: %f\n", elapsed_time(exponential, sum_time));
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(sumWeights, weights) private(x))
		for(x = 0; x < Nparticles; x++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for private(x) reduction(+:xe, ye))
		for(x = 0; x < Nparticles; x++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(u, u1, Nparticles) private(x))
		for(x = 0; x < Nparticles; x++){
		...
		#pragma omp metadirective when(user={condition(Nparticles>100)} : parallel for shared(CDF, Nparticles, xj, yj, u, arrayX, arrayY) private(i, j))
