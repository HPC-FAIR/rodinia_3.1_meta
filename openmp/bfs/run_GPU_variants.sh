...
do
  ...
  TESTINPUT="${TESTINPUT%.*}" 
  echo "Test command: $GPUPERFTOOL $GPUPERFFLAG -u ms --log-file $LOGDIR/${t}_${TESTINPUT}.log $app $t $inputdata"
  $GPUPERFTOOL $GPUPERFFLAG -u ms --log-file "$LOGDIR/${t}_${TESTINPUT}.log" $app $t $inputdata &> tmp.log
